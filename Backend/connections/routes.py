...
def Routes():
    ...
    async def process_exercise_video(request: Request, submission_id: int):
        ...
        try:
            ...
            try:
                ...
                # Get path to the original video
                ...
                # Only consider a valid processed video if it has content
                if file_exists and file_size > 0:
                    # Verify the file is not being processed
                    ...
                    # Video is already processed, return info for download endpoint
                    return JSONResponse(content={
                ...
                # Initialize app-level storage for processing state if needed
                ...
                # If there's an invalid processed video, remove it
                if file_exists and file_size == 0:
                    try:
                ...
                # Create a stop event for this job
    ...
    async def get_processed_video_status(request: Request, submission_id: int):
        ...
        try:
            ...
            # Get path to the processed video
            db = get_Mysql_db()
            cursor = db.cursor(dictionary=True)
            try:
                cursor.execute(
                ...
                # Only consider a video ready if the file exists, has content, and processing is not active
    ...
    # Helper function to estimate processing completion percentage
    ...
    async def stop_exercise_video_processing(request: Request, submission_id: int):
        ...
        try:
            ...
            # Set the stop event for this submission
            stopped = False
            if hasattr(app, "video_stop_events") and submission_id in app.video_stop_events:
                ...
                # Wait for the processing to acknowledge the stop signal
                for i in range(3):  # Try up to 3 seconds
                    time.sleep(1)
            ...
            try:
                ...
                # Generate token for video if it exists
    ...
    def process_video_with_pose_detection(video_path, submission_id=None):
        try:
            ...
            # Remove any existing incomplete file first
            ...
            with mp_pose.Pose(
                ...
                while cap.isOpened():
                    ...
                    if total_frames > 0 and submission_id is not None:
                        ...
                        app.processing_progress[submission_id] = min(progress, 99)  # Cap at 99% until fully done
            ...
            # Verify the file exists and has content before setting permissions
            ...
            # Remove from tracking dictionaries after completion
    ...
    async def download_video(request: Request, filename: str, submission_id: int):
        ...
        try:
            ...
            try:
                ...
                # Build the file path based on the submission
    ...
    async def regenerate_exercise_video(request: Request, submission_id: int):
        ...
        try:
            ...
            try:
                ...
                if hasattr(app, "video_stop_events") and submission_id in app.video_stop_events:
                    app.video_stop_events[submission_id].set()
                    # Give time for the thread to stop
                    time.sleep(1)
